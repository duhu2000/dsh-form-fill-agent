...
for (const entry of [process.env.DSH_RC_BIN, process.env.DSH_ALPHA_BIN].filter(Boolean)) {
 ...
 const sidebarVersion=process.env.SIDEBAR_VERSION;
 ...
 if(sidebarVersion)run(['plugin','--profile','web','add','dsh-better-sidebar@'+sidebarVersion,'--ignore-scripts','--registry=https://registry.npmjs.org/','--store-dir',join(home,'pnpm-store')]);
