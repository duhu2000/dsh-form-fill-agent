...
try{
 ...
 const bytes=fixtureBytes('合成表',['企业名称','地址','法定代表人','企业状态','CreditCode','核准日期 YYYY-MM-DD','受益人','主营业务收入','注册资本（重复列 2）'],Array.from({length:60},(_,i)=>['合成企业'+i+'有限公司','','','','','','','','']),{title:false});
 ...
 assert.match(await page.locator('#business-summary').textContent(),/480 个待补空位置/);
