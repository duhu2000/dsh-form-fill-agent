...
当前为 0.1.0-alpha.1 预览版本。支持简单 XLSX、确定性同义表头、企业完整登记名、六类工商字段、来源追溯、未完成清单和任务恢复。没有 LLM 猜值；已有内容默认保留。
...
2. 上传简单 XLSX。首次分析只用合成 Provider，不发起企查查调用。
3. 需要真实数据时，复制工作台的查询指令到 DSH 对话。Agent 调用 form_fill_enrich，嵌套调用已连接的企查查企业工商 MCP。
4. 点击刷新任务，查看各格准备填写的事实值、来源和未完成项。
