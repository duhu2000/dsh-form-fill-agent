...
当前正式编号版本为 **0.2.6**，包含完整网格、恢复选择、取消/重试、129 字段目录、受限 Excel 保真及企查查蓝工作台。默认只填空白，来源可追溯，支持任务历史和中断恢复；不使用 LLM 猜值。发布记录见 [0.2.6](docs/RELEASE-0.2.6.md)，正式用户签收与市场上架状态见 [当前进度](docs/PROGRESS.md)。
...
插件清单在 packages/dsh-form-fill-agent/package.json，主仓是 private npm monorepo。主包 0.2.6 精确依赖内核及 Provider 0.2.0；推荐使用 latest 渠道。
