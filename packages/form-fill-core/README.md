...
analyzeDocument 的第三个参数可包含 sheets（工作表名数组）、headers（sheet/row）、mappings（sheet/column/field；null 为排除）、anchors（sheet/row/value；人工核验的主体）。字段目录可声明 anchorFallback，在没有主锚点列时使用备用身份列；内核不硬编码 Provider 字段名。设置不会改写输入单元格；变更后的计划须重新预览并确认。
