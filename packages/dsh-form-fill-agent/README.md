...
补充要求为模型可读文本；字段或工作表范围请通过向导/工作台修改，不能仅改文字后假定执行范围已同步。任务 schema 4 可读取旧任务；回退 alpha.5 或更早版本请使用独立任务目录。共享内核和 Provider 仍为 alpha.5。

支持企业完整登记名称或 18 位信用代码；简称、主体不一致或多候选不会自动猜选。目前没有模糊候选搜索工具。字段映射采用确定性规则及人工确认，不让模型编造事实。公式、区域引用下拉验证、复杂 Excel 结构和 Word 暂不支持，保真边界见下方安全说明。
...
alpha.5 保存任务 schema 3，支持读取旧 schema 1/2；回退旧版本须使用升级前任务目录副本或新目录，防止丢失已确认的字段/主体设置。
