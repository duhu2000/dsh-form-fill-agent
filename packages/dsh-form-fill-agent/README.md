...
5. 确认后下载新 XLSX 副本和独立 XLSX 任务结果报告。原文件不覆盖，已有内容默认保留。
...
补充要求为模型可读文本；字段或工作表范围请通过向导/工作台修改，不能仅改文字后假定执行范围已同步。任务 schema 5 可读取 schema 1–5；回退旧版本请使用升级前目录副本或独立任务目录。主包为 0.2.31，共享内核为 0.2.10，Provider 为 0.2.2，共享字段契约为 0.1.0。
