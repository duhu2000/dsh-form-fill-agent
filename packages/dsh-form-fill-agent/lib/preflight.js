...
export function assessCompatibility({nodeVersion,hostVersion,sidebarVersion,contextVersion,mode='basic'}) {
 if(!['basic','workbench'].includes(mode))throw Error('mode 仅支持 basic 或 workbench');
 const issues=[];
 ...
 if(!sidebarVersion)add(mode==='workbench'?'blocked':'info','SIDEBAR_MISSING','未安装侧栏：基础会话与独立 /form-fill/ 页面可用；内嵌工作台及自动草稿回填需安装与宿主匹配的 Better Sidebar。');
 ...
 const known=(!sidebarVersion&&['0.1.1-rc.2','0.1.2-rc.1'].includes(hostVersion))||hostVersion==='0.1.1-rc.2'&&sidebarVersion==='0.17.1'||hostVersion==='0.1.2-rc.1'&&sidebarVersion==='0.18.1';
 if(!known)add('warning','COMBINATION_UNVERIFIED','此宿主与侧栏组合未验证；版本范围可接受不等于已通过业务验收。');
 const blocked=issues.some(i=>i.severity==='blocked');
 return {status:blocked?'blocked':issues.some(i=>i.severity==='warning')?'unverified':'verified-combination',mode,nodeVersion,hostVersion,sidebarVersion:sidebarVersion||null,contextVersion:contextVersion||null,issues,scope:'版本预检；不代表模型、MCP 权限或全部业务验收通过。不会自动安装、升级、重启。'};
}

export async function inspectInstallation({dshBin,profileDir,sidebarVersion,contextVersion,mode='basic'}) {
 ...
 return {...assessCompatibility({nodeVersion:process.version,hostVersion,sidebarVersion:sidebarVersion||installedSidebar,contextVersion:contextVersion||installedContext,mode}),installed:{sidebar:installedSidebar||null,context:installedContext||null},proposed:{sidebar:sidebarVersion||null,context:contextVersion||null}};
}

if(process.argv[1]&&await realpath(process.argv[1]).catch(()=>resolve(process.argv[1]))===fileURLToPath(import.meta.url)) {
 try {
  const args=process.argv.slice(2),options={};
  if(args.includes('--help')){console.log('form-fill-preflight --dsh-bin /absolute/path/to/dsh --profile-dir /path/to/profiles/web [--mode basic|workbench] [--sidebar-version 0.18.1] [--context-version 0.48.0]\n只读当前版本；可预检计划安装版本。阻断退出码 2，未验证组合会显式提示。');}
  else {
   const keys={'--mode':'mode','--dsh-bin':'dshBin','--profile-dir':'profileDir','--sidebar-version':'sidebarVersion','--context-version':'contextVersion'};
