...
export function createTaskStore({ directory, maxTasks = 10, now = Date.now, ttlMs = 86400000 } = {}) {
  ...
  if (directory) {
    ...
    try {
      for (const file of readdirSync(root)) {
        ...
        if (![1,2].includes(saved.schema) || saved.id !== id || !Number.isFinite(saved.created)) throw Error('Invalid task schema');
        ...
        const task = { bytes, preview: saved.preview, created: saved.created, revision: saved.revision ?? 1, owner: saved.owner, filename: saved.filename ?? '未命名表格.xlsx', updatedAt: saved.updatedAt ?? saved.created, state: saved.state === 'enriching' ? 'interrupted' : saved.state ?? (saved.confirmed ? 'completed' : 'preview_ready'), sessionId: saved.sessionId };
  ...
  return {
    ...
    set(id, task) {
      ...
      if (root) {
        const json = JSON.stringify({ schema:2,id,base64:task.bytes.toString('base64'),preview:task.preview,created:task.created,revision:task.revision ?? 1,confirmed:!!task.result,owner:task.owner,filename:task.filename,updatedAt:task.updatedAt,state:task.state,sessionId:task.sessionId });
