...
AI填表的企查查 Provider，包含 129 字段目录、合成演示 Provider，以及由调用方授权 transport 的工商和多域 Provider。
...
Node.js 22 或以上。导出 FIELD_CATALOG（129 字段）、createMockProvider、createQccProvider（工商）和 createCatalogProvider（按可用工具启用扩展）。真实 Provider 由调用方注入 callTool，不包含访问凭据。
...
createCatalogProvider 通过 availableTools 显式启用企业简介、联系方式、上市、开票、进出口和两类风险扫描；先核验身份，再读取对应工具。确定性投影来源路径含 /projection/，保留旧 128 字段契约并适配新增关联惩戒名单维度。风险计数仅描述公开记录，不代表风险结论；未知目录变化会阻断该组填写。缺失工具不影响插件加载。
