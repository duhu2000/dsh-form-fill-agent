...
当前主包与共享内核正式版本 **0.2.8**，Provider 保持 **0.2.0**，均已发布 npm。本轮修复重复值条件格式、任意值校验及静态整数校验的导入；整数校验区域暂不自动填写。103 项测试、六组 CI、tarball 消费兼容、24 golden parity、三模板端到端及双宿主隔离安装卸载通过。详见 [RELEASE-0.2.8.md](RELEASE-0.2.8.md)。用户样表已只读验证可分析；不代表全部字段已填写、Excel 桌面正式验收或市场已上架。
